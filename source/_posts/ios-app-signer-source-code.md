...
在之前的制作[非越狱抢红包插件]()文章中，我们曾经使用过 [ios-app-signer](https://github.com/buginux/ios-app-signer) 对微信进行过重签名。`ios-app-signer` 提供了简单易用的图形界面来帮助我们进行重签名，但是我们今天要透过它的图形界面，深入到源码中学习下 iOS 应用重签名的原理。

<!-- more -->

首先，从 `ios-app-signer` 的 [Github 仓库]()上将其 `clone` 下来。
